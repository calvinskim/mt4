create or replace table `marketeam_data.health2_data_temp`
(account_id string,
gift_date date,
...
(with temp as
  (
    ...
    left join
      `marketeam_data.health2_promotions` p
      on g.AccountID = p.AccountID
      and g.AppealCode = p.AppealCode
    where
      g.AccountID in (select account_id from `marketeam_data.health2_samples`)
  ),
temp2 as 
  (
    select

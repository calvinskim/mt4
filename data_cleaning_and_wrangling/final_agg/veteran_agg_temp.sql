create or replace table `marketeam_data.veteran_data_temp`
(account_id string,
gift_date date,
...
(with temp as
  (
    ...
    left join
      `marketeam_data.veteran_promotions` p
      on g.AccountID = p.AccountID
      and g.AppealCode = p.AppealCode
    where
      g.AccountID in (select account_id from `marketeam_data.veteran_samples`)
      and length(g.GiftDate) = 10
  ),
temp2 as 
  (
    select

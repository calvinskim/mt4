create or replace table `marketeam_data.advcy_data_temp`
(account_id string,
gift_date date,
...
(with temp as
  (
    ...
    left join
      `marketeam_data.advcy_promotions` p
      on g.AccountID = p.AccountID
      and g.AppealCode = p.AppealCode
    where
      length(g.GiftDate) = 10 
      --and date(g.GiftDate) >= date('2017-01-01')
      --and g.AccountID = '1032807C'
      and g.AccountID in (select account_id from `marketeam_data.advcy_samples`)
  ),
temp2 as 
  (
    select
